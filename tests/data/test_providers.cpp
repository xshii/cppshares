...
TEST_F(ProvidersTest, EastMoneyProvider_DataFetching) {
    ...
    std::vector<Symbol> test_stocks = {
        ...
        Symbol("000001", Market::SZ, SecurityType::STOCK),  // 平安银行
        Symbol("600036", Market::SH, SecurityType::STOCK),  // 招商银行
    };

    if (is_running_in_ci()) {
        std::cout << "Skipping network tests in CI environment" << std::endl;
        return;
    }
    ...
    for (const auto& symbol : test_stocks) {
        ...
        if (tick.has_value()) {
            ...
            std::cout << "  Price: " << tick->price << ", Volume: " << tick->volume << std::endl;
        } else {
            std::cout << "  No realtime data available" << std::endl;
        }
        ...
        if (!klines.empty()) {
            ...
            EXPECT_GT(latest.high, 0.0);
            EXPECT_GT(latest.low, 0.0);
            EXPECT_GT(latest.close, 0.0);
        }
    }
}
...
// 多平台数据对比测试
TEST_F(ProvidersTest, AllProviders_DataComparison) {
    std::vector<Symbol> test_stocks = {
        Symbol("600597", Market::SH, SecurityType::STOCK),  // 上海悦达
        Symbol("000001", Market::SZ, SecurityType::STOCK),  // 平安银行
    };
    ...
    for (const auto& symbol : test_stocks) {
        ...
        for (const auto& tick : ticks) {
            if (tick.has_value() && tick->price > 0) {
                valid_prices.push_back(tick->price);
            }
        }
