...
public:
    explicit binary_sink(const std::string& filename)
        : binary_file_(filename, std::ios::binary | std::ios::app)
        , write_buffer_(BUFFER_SIZE) {}

protected:
    void sink_it_(const spdlog::details::log_msg& msg) override {
        BinaryLogEntry header{
            .timestamp_us = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::microseconds>(
...
public:
    ...
    // 便利方法：记录订单
    void log_order(uint64_t order_id, uint32_t symbol_id, double price, 
                  uint32_t quantity, bool is_buy, uint8_t status) {
        OrderRecord record{
            .order_id = order_id,
            .symbol_id = symbol_id,
            .price = price,
    ...
    // 便利方法：记录策略信号
    void log_strategy_signal(uint32_t strategy_id, uint32_t symbol_id, 
                           uint8_t signal_type, uint8_t confidence,
                           double target_price, uint32_t target_quantity) {
